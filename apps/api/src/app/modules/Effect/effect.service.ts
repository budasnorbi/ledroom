...
export class EffectService {
  ...
  public step(
    ...
    const { ledColors, barCount, speed, direction, rangeStart, rangeEnd } =
      config;
  ...
  public blink(config: Blink) {
    ...
    for (let i = 0; i < this.ledCount; i++) {
      ...
      // Red
      frame[rgbIndex] = Math.floor(
        map(transitionPct, 0, 100, frameRed, toColor[0])
      );
      // Green
      frame[rgbIndex + 1] = Math.floor(
        map(transitionPct, 0, 100, frameGreen, toColor[1])
      );
      // Blue
      frame[rgbIndex + 2] = Math.floor(
        map(transitionPct, 0, 100, frameBlue, toColor[2])
      );
    }
